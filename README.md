...
达尔文.skill — 一个让Skill无限进化的系统：评估→改进→测试→保留或回滚。

- **作者**: 花叔 (Alchain)
- **许可证**: [MIT License](https://github.com/alchaincyf/darwin-skill/blob/master/LICENSE)
- **灵感来源**: 受 Andrej Karpathy 的 autoresearch 启发
- **学术基础**: 吸收微软研究院 SkillLens (arXiv:2605.23899) 与 SkillOpt (arXiv:2605.23904) 论文
- **用途**: 本项目的 `SKILL.md` 和进化机制参考了达尔文.skill 的9维度评估体系

感谢 Simon 林 和 花叔 开源的优质项目！

## 达尔文进化机制

本框架采用达尔文.skill 的进化机制进行Skill优化：
...
```bash
# 运行进化脚本
python scripts/evolve.py --skill SKILL.md --test test-prompts.json --rounds 10
```
